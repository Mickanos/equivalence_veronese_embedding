...
gln_sln := function(k, n)
  ...
  L, phi := LieAlgebra(gln);
  sln, proj := quo<L | Basis(Center(L))>;
  lifts := [M - (Trace(M)/n) * One(gln) where M is S @@ (phi * proj) :
    S in Basis(sln)];
  lift_map := hom< sln -> L | lifts @ phi>;
  return gln, sln, phi * proj, lift_map * Inverse(phi);
end function;
...
EnvelopingAlgebra := function(L)
  ...
  gln, sln, _, lift_sln := gln_sln(K, n);
